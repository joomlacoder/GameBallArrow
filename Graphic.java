...
public class Graphic {
    ...
    private Graphics g;
    private CopyOnWriteArrayList<DrowObj> drowObjs;
    ...
    Graphic(String nameWindow, int width, int higth, Canvas content, CopyOnWriteArrayList<DrowObj> drowObjs){
        initWindow(nameWindow, width, higth, content);
        initGraphic(width, higth, content);
        this.drowObjs = drowObjs;
    }

    void draw(Graphics g) {
        drowObjs.stream().forEach(x -> x.drow(g));
    }
